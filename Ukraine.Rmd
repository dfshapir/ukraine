...
```{r hdireal}
City <- City %>%
  filter(`Oblast Center` == 1) %>%

# Drop Kramatorsk due to missing data
  
  filter(City != "Kramatorsk") %>%
...
```{r regression}
hdireg <- lm(Percentage ~ `2019 Oblast HDI` + `2020 City Population` + `Total Ukr Cases`, data = City)
